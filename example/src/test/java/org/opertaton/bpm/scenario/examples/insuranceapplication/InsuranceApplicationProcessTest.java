package org.opertaton.bpm.scenario.examples.insuranceapplication;
...
import static org.operaton.bpm.engine.test.assertions.ProcessEngineTests.runtimeService;
import static org.operaton.bpm.engine.test.assertions.ProcessEngineTests.withVariables;
...
@Deployment(resources = {
        "org/opertaton/bpm/scenario/examples/insuranceapplication/InsuranceApplication.bpmn",
        "org/opertaton/bpm/scenario/examples/insuranceapplication/DocumentRequest.bpmn",
        "org/opertaton/bpm/scenario/examples/insuranceapplication/RiskCheck.dmn"
})
public class InsuranceApplicationProcessTest {
  ...
  void setupDefaultScenario() {
    ...
    when(insuranceApplication.waitsAtUserTask("UserTaskDecideAboutApplication")).thenReturn((task) -> task.complete(withVariables("approved", true)));
    ...
    when(documentRequest.waitsAtUserTask("UserTaskCallCustomer")).thenReturn(TaskDelegate::complete);
  ...
  void testYellowScenario() {
    ...
    Scenario scenario = Scenario.run(insuranceApplication)
      .startBy(() -> { // ... or define your own starter function
        return runtimeService().startProcessInstanceByKey("InsuranceApplication", variables);
      })
      .execute();
  ...
  @Test
  void testManualApprovalScenario() {
  ...
  @Test
  void testManualRejectionScenario() {
    ...
    when(insuranceApplication.waitsAtUserTask("UserTaskDecideAboutApplication")).thenReturn((task) -> task.complete(withVariables("approved", false)));
  ...
  public void testDocumentRequestBitLateScenario() {
    ...
    when(documentRequest.waitsAtReceiveTask("ReceiveTaskWaitForDocuments")).thenReturn((receiveTask) -> receiveTask.defer("P1DT1M", receiveTask::receive));
  ...
  public void testDocumentRequestVeryLateScenario() {
    ...
    when(documentRequest.waitsAtReceiveTask("ReceiveTaskWaitForDocuments")).thenReturn((receiveTask) -> receiveTask.defer("P7DT1M", receiveTask::receive));
