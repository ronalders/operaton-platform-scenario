...
import org.operaton.bpm.scenario.impl.util.Api;
import org.operaton.bpm.scenario.impl.util.IdComparator;
import org.operaton.bpm.scenario.impl.util.Log;
import org.operaton.bpm.scenario.impl.waitstate.CallActivityExecutable;
import org.operaton.bpm.scenario.run.ProcessRunner;
import org.operaton.bpm.scenario.run.ProcessRunner.ExecutableRunner.StartingByKey;
import org.operaton.bpm.scenario.run.ProcessRunner.ExecutableRunner.StartingByMessage;
import org.operaton.bpm.scenario.run.ProcessRunner.ExecutableRunner.StartingByStarter;
import org.operaton.bpm.scenario.run.ProcessRunner.StartableRunner;
import org.operaton.bpm.scenario.run.ProcessStarter;
...
public class ProcessRunnerImpl extends AbstractRunner implements StartingByKey, StartingByMessage, StartableRunner, StartingByStarter, ProcessRunner {
  ...
  public StartingByKey fromBefore(String activityId) {
    Api.feature(RuntimeService.class.getName(), "createProcessInstanceByKey", String.class)
      .fail("Outdated Operaton BPM version used will not allow to start process instances " +
  ...
  public ProcessInstance run() {
    if (this.processInstance == null) {
      if (this.processDefinitionKey != null) {
        ...
        this.processStarter = () -> scenarioExecutor.processEngine.getRuntimeService().startProcessInstanceByMessage(startMessage, variables);
      }
    }
  ...
  public void setExecuted() {
    boolean supportsCanceled = Api.feature(HistoricActivityInstance.class.getName(), "isCanceled")
      .warn("Outdated Operaton BPM version used will not allow to use " +
